...
<span class="text-blue font-semibold">Throughput</span> denotes the speed at
which a Java application runs. For example, if your application is a
transaction-based system, high throughput means that more transactions are
executed during a given amount of time.

<span class="text-rose font-semibold">Pause time</span> is the duration during
which the application is temporarily frozen while the GC is running. For
example, if you are playing Minecraft and the screen suddenly freezes for 0.5
seconds, it means that the GC is running.

<span class="text-blue">Throughput</span> and
<span class="text-rose">pause time</span> are inversely proportional to each
other. They key in here is finding the equilibrium between them.
